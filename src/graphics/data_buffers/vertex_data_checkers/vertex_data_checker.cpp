...
namespace graphics {
bool VertexDataChecker::AreIndicesValid(
  ...
  for (size_t i = 0; i < vertex_data.indices.size(); i += 3)
    if (vertex_data.indices[i] == vertex_data.indices[i + 1] ||
      ...
      return false;
  GLuint number_of_vertices = vertex_data.vertices.size() /
                              std::accumulate(vertex_data.data_format.begin(),
...
bool VertexDataChecker::AreVerticesValid(
    const VertexData &vertex_data) const {
